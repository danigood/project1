...
xbarsuper <- c(mean(atc24_superdata$quantity), mean(b624_superdata$quantity), mean(atc48_superdata$quantity), mean(b648_superdata$quantity), mean(atc72_superdata$quantity),mean(b672_superdata$quantity))
sdsuper <- c(sd(atc24_superdata$quantity), sd(b624_superdata$quantity), sd(atc48_superdata$quantity), sd(b648_superdata$quantity), sd(atc72_superdata$quantity),sd(b672_superdata$quantity))
...
mpsuper <- barplot(data,beside=T,col=c("blue","orange"),main="Supernatant Fraction Data",xlab="Hours Post Infection",ylab="Genome Copies per 4 µL Supernatant",ylim=c(0,1e7))
segments(mpsuper, xbarsuper - sdsuper, mpsuper, xbarsuper + sdsuper, lwd=2)
segments(mpsuper - 0.1, xbarsuper - sdsuper, mpsuper + 0.1, xbarsuper - sdsuper, lwd=2)
segments(mpsuper - 0.1, xbarsuper + sdsuper, mpsuper + 0.1, xbarsuper + sdsuper, lwd=2)
```
